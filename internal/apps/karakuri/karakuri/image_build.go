...
func buildProcCreateBuildBook() BuildBook {
	// read build file
	build_commands := readBuildFile()
	// create build book
...
func buildProcCreateContainer(build_book BuildBook, image_id string) string {
	...
	if res, resp_id, message := requestContainerId(container_name); !res {
		fmt.Println(message)
		return ""
	} else {
...
func BuildImage(image string) {
	...
	build_book := buildProcCreateBuildBook()
	fmt.Printf("Done.\n")
