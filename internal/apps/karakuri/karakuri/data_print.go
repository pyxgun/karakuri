...
func printContainerList(container_list hitoha.ContainerList, namespace string) {
	...
	fmt.Println("[Node] " + strings.Split(node, ":")[0] + "\n")
	if namespace != "all" {
...
func ShowContainerLog(id string, name string) {
	...
	node := getTargetNode().Target
	if node != "localhost:9806" {
		fmt.Println("'karakuri logs' command is not available for remote node")
		os.Exit(1)
	}
	container_id := id
	if name != "none" {
	...
	cmd := exec.Command("cat", karakuripkgs.FUTABA_ROOT+"/"+container_id+"/container.log")
	cmd.Stdin = os.Stdin
...
// node controller
func printTargetNode(node_info NodeInfo) {
	fmt.Println("Mode : " + node_info.Mode)
	fmt.Println()
	fmt.Println("Target Node : " + strings.Split(node_info.Target, ":")[0])
	fmt.Println("Status      : " + node_info.Status)
}

func printNodeList(node_list RemoteNodeList, connected_node NodeInfo) {
	fmt.Println("[Node List]")
	for _, node := range node_list.List {
		if connected_node.Target == node.Node && connected_node.Status == "connected" {
			fmt.Println("  *  " + strings.Split(node.Node, ":")[0])
		} else {
			fmt.Println("     " + strings.Split(node.Node, ":")[0])
		}
	}
