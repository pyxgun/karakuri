...
func GetRemoteNode() {
	...
	connected_node := getTargetNode()
	printNodeList(remote_node_list, connected_node)
}
...
func EnableRemoteControllMode() {
	...
	// accept
	iptables_cmd := exec.Command("iptables", "-D", "INPUT", "-p", "tcp", "-i", karakuripkgs.HOST_NIC, "-d", karakuripkgs.HOST_ADDR, "--dport", "9816", "-j", "DROP")
	...
	fmt.Println("  karakuri nodectl connect --node " + karakuripkgs.HOST_ADDR + " --auth " + auth_code)
}
...
func DisableRemoteControllMode() {
	...
	// accept
	iptables_cmd := exec.Command("iptables", "-A", "INPUT", "-p", "tcp", "-i", karakuripkgs.HOST_NIC, "-d", karakuripkgs.HOST_ADDR, "--dport", "9816", "-j", "DROP")
	if err := iptables_cmd.Run(); err != nil {
